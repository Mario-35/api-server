...
export function auth(req: Request, res: Response): void {
  const authorizeUrl = client.generateAuthUrl({
    ...
    redirect_uri: env.isProduction ? undefined : `${req.protocol}://${req.get("host")}${req.path}/return`, // TEMP
  });
...
export async function callback(req: Request, res: Response): Promise<void> {
  try {
    const { tokens: credentials } = await client.getToken({
      ...
      redirect_uri: env.isProduction ? undefined : `${req.protocol}://${req.get("host")}${req.path}`, // TEMP
    });
