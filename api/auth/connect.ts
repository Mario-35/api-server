...
export default async function connect(
  ...
  if (user) {
    ...
    const dbIdentity = await db.table<Identity>("identities").where(key).first();

    if (dbIdentity) {
      if (dbIdentity.user_id !== user.id) {
        throw new Error("These credentials already linked to another user account.");
      }
