...
export class Validator<
  ...
  Output extends Record<string, any> = {} // eslint-disable-line @typescript-eslint/ban-types
> {
  ...
  private validate: (cb: (value: any, field: string) => Status) => this = () => this;
  ...
  field<Key extends keyof Input, As extends string | undefined>(
  ...
  ): Validator<Input, Record<keyof Output | ([undefined] extends [As] ? Key : As), any>> {
    ...
    if (value && config.type) {
      ...
      if (globalId.type !== config.type) {
        throw new Error(`Expected an ID of type '${config.type}' but got '${globalId.type}'.`);
      }
  ...
  isFloat = (options?: validator.IsFloatOptions, message?: string): this =>
    this.validate((value) => ({
      valid: value === undefined ? true : isFloat(value, options),
      message: message || "The Float is invalid.",
    }));

  isInt = (options?: validator.IsIntOptions, message?: string): this =>
    this.validate((value) => ({
      valid: value === undefined ? true : isInt(value, options),
      message: message || "The Integer is invalid.",
    }));
}

export function validate<Input extends Record<string, any>, Validation>(
  input: Input,
